...
const AuthenticationController = {
  /**
	 ...
	 *          validation result, errors tips, and a global message for the
	 *          whole form.
	 */
  ...
  forgot: (req, res, next) => {
    ...
    return User.findOne({ $or: [ {username: req.body.username},{email: req.body.username} ], isActive: true }, (err, user) => {
        ...
        var resetCode = generator.generate({
            length: 16,
            numbers: true,
        ...
        let now = new Date();
        user.passwordResetExpiry = new Date(now.getTime() + 30*60000); //30 minutes expiry
        user.save((err, user) => {
            if (err) {
  ...
  passwordReset: (req, res, next) => {
	    ...
	    return User.findOne({passwordResetCode: req.body.resetCode , username: req.body.username, passwordResetExpiry: {$gt: Date.now()} , isActive: true }, (err, user) => {
